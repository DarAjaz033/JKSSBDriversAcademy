...
async function scanAndInjectButtons() {
    ...
    links.forEach(async (link) => {
        ...
        if (sizeText) {
            ...
            if (titleSpan && !titleSpan.innerHTML.includes('MB') && !titleSpan.innerHTML.includes('KB')) {
                titleSpan.innerHTML += ` <strong style="font-size:11px; color:#e07b2a; opacity:0.9;">(${sizeText})</strong>`;
            } else if (!titleSpan && !link.innerHTML.includes('MB')) {
                link.innerHTML += ` <strong style="font-size:11px; color:#e07b2a; opacity:0.9;">(${sizeText})</strong>`;
            }
        }
