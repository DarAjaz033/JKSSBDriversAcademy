import{d as b,c as g,e as p}from"./firebase-config-BMA5n-tq.js";/* empty css               */import{e as v}from"./escape-html-BUkjI-KV.js";class f{constructor(){this.test=null,this.currentQuestionIndex=0,this.userAnswers=[],this.currentState="LOBBY",this.timerInterval=null,this.timeRemaining=0,this.handleFullscreenChange=()=>{!document.fullscreenElement&&this.currentState==="IN_PROGRESS"&&(this.pauseTimer(),this.showAntiCheatWarning())},this.testContent=document.getElementById("test-content"),this.init()}async init(){const e=new URLSearchParams(window.location.search).get("id");if(!e){this.testContent.innerHTML='<div style="text-align: center; padding: var(--spacing-xl);">Test not found</div>';return}await this.loadTest(e)}async loadTest(t){try{const e=b(g,"practiceTests",t),s=await p(e);if(s.exists()){this.test={id:s.id,...s.data()},this.userAnswers=new Array(this.test.questions.length).fill(null);const i=this.test.questions.length;this.timeRemaining=i*60,this.render()}else this.testContent.innerHTML='<div style="text-align: center; padding: var(--spacing-xl);">Test not found</div>'}catch{this.testContent.innerHTML='<div style="text-align: center; padding: var(--spacing-xl);">Error loading test</div>'}}render(){if(this.test)switch(this.currentState==="IN_PROGRESS"?document.body.classList.add("is-testing"):document.body.classList.remove("is-testing"),this.currentState){case"LOBBY":this.renderLobby();break;case"IN_PROGRESS":this.renderActiveTest();break;case"REVIEW":this.renderActiveTest(!0);break;case"RESULTS":this.renderResults();break}}renderLobby(){var s;if(!this.test)return;const t=this.test.questions.length,e=t*1;this.testContent.innerHTML=`
    ...
    `,(s=document.getElementById("start-test-btn"))==null||s.addEventListener("click",()=>this.startTest()),window.lucide.createIcons()}async startTest(){try{document.documentElement.requestFullscreen&&await document.documentElement.requestFullscreen()}catch(t){console.warn("Fullscreen API block:",t)}document.addEventListener("fullscreenchange",this.handleFullscreenChange),this.currentState="IN_PROGRESS",this.startTimer(),this.render()}showAntiCheatWarning(){var e,s;let t=document.getElementById("warn-modal");t||(t=document.createElement("div"),t.id="warn-modal",t.className="modal-overlay",t.innerHTML=`
      ...
      `,document.body.appendChild(t),window.lucide.createIcons(),(e=document.getElementById("warn-submit-btn"))==null||e.addEventListener("click",()=>{t==null||t.remove(),this.submitTest()}),(s=document.getElementById("warn-resume-btn"))==null||s.addEventListener("click",async()=>{try{document.documentElement.requestFullscreen&&await document.documentElement.requestFullscreen()}catch{}t==null||t.classList.remove("active"),setTimeout(()=>t==null?void 0:t.remove(),300),this.startTimer()})),requestAnimationFrame(()=>{t==null||t.classList.add("active")})}startTimer(){this.timerInterval!==null&&window.clearInterval(this.timerInterval),this.timerInterval=window.setInterval(()=>{this.timeRemaining--,this.updateTimerUI(),this.timeRemaining<=0&&(this.timerInterval!==null&&window.clearInterval(this.timerInterval),this.submitTest())},1e3)}pauseTimer(){this.timerInterval!==null&&window.clearInterval(this.timerInterval)}updateTimerUI(){const t=Math.floor(this.timeRemaining/60).toString().padStart(2,"0"),e=(this.timeRemaining%60).toString().padStart(2,"0"),s=document.getElementById("dash-timer-txt");if(s){s.textContent=`${t}:${e}`;const i=document.getElementById("dash-timer-wrap");this.timeRemaining<=60&&i&&i.classList.add("dash-time-warn")}}renderActiveTest(t=!1){if(!this.test)return;const e=this.test.questions[this.currentQuestionIndex],s=this.currentQuestionIndex===0,i=this.currentQuestionIndex===this.test.questions.length-1,n=this.userAnswers[this.currentQuestionIndex],a=e.correctAnswer;let o="";if(t)o=`
      ...
      `;else{const m=Math.floor(this.timeRemaining/60).toString().padStart(2,"0"),r=(this.timeRemaining%60).toString().padStart(2,"0");o=`
        <div class="test-dash-header">
          ...
          <div class="dash-timer ${this.timeRemaining<=60?"dash-time-warn":""}" id="dash-timer-wrap">
            <i data-lucide="timer" width="18"></i> <span id="dash-timer-txt">${m}:${r}</span>
          </div>
        </div>
      `}const l=e.options.map((m,r)=>{let c="";if(t){r===a?c="correct":r===n&&r!==a&&(c="wrong");let h="";return r===a?h='<span style="margin-left:auto; color:#16A34A; font-weight:700; font-size:12px;">Correct Answer</span>':r===n&&(h='<span style="margin-left:auto; color:#DC2626; font-weight:700; font-size:12px;">Your Answer</span>'),`
          <div class="option-item ${c}" style="cursor: default;">
            <div class="option-radio ${n===r?"selected":""}"></div>
            <div class="option-text">${v(m)}</div>
            ${h}
          </div>
        `}else return c=n===r?"selected":"",`
          <div class="option-item selectable-option ${c}" data-index="${r}">
            <div class="option-radio ${n===r?"selected":""}"></div>
            <div class="option-text">${v(m)}</div>
          </div>
        `}).join(""),u=`
    ...
    `;let d="";!t&&i?d=`
      ...
      `}
    `,this.attachActiveTestListeners(t),window.lucide.createIcons()}attachActiveTestListeners(t){var e,s,i,n,a;t||document.querySelectorAll(".selectable-option").forEach(o=>{o.addEventListener("click",l=>{const u=parseInt(l.currentTarget.getAttribute("data-index"));this.userAnswers[this.currentQuestionIndex]=u,this.render()})}),(e=document.getElementById("prev-btn"))==null||e.addEventListener("click",()=>{this.currentQuestionIndex>0&&(this.currentQuestionIndex--,this.render())}),(s=document.getElementById("next-btn"))==null||s.addEventListener("click",()=>{this.currentQuestionIndex<this.test.questions.length-1&&(this.currentQuestionIndex++,this.render())}),(i=document.getElementById("leave-test-btn"))==null||i.addEventListener("click",()=>this.confirmLeave()),(n=document.getElementById("exit-review-btn"))==null||n.addEventListener("click",()=>{this.currentState="RESULTS",this.render()}),(a=document.getElementById("submit-test-btn"))==null||a.addEventListener("click",()=>this.confirmSubmit())}confirmLeave(){this.pauseTimer(),confirm("Are you sure you want to completely leave this test? Your progress will be lost.")?(document.fullscreenElement&&document.exitFullscreen().catch(e=>console.warn(e)),window.location.href="mock-tests.html"):this.startTimer()}confirmSubmit(){var s,i;this.pauseTimer();let t=this.userAnswers.filter(n=>n===null).length,e=document.createElement("div");e.className="modal-overlay",e.innerHTML=`
      <div class="modal-content">
    ...
    `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("active")),(s=document.getElementById("conf-cancel-btn"))==null||s.addEventListener("click",()=>{e.classList.remove("active"),setTimeout(()=>e.remove(),300),this.startTimer()}),(i=document.getElementById("conf-submit-btn"))==null||i.addEventListener("click",()=>{e.classList.remove("active"),setTimeout(()=>e.remove(),300),this.submitTest()})}submitTest(){this.timerInterval!==null&&window.clearInterval(this.timerInterval),document.removeEventListener("fullscreenchange",this.handleFullscreenChange),this.currentState="RESULTS",this.render()}renderResults(){var a,o;if(!this.test)return;let t=0,e=0,s=0;this.test.questions.forEach((l,u)=>{const d=this.userAnswers[u];d===null?s++:d===l.correctAnswer?t++:e++});const i=t*1-e*.25,n=this.test.questions.length;this.testContent.innerHTML=`
      <div class="results-card">
        ...
        <div style="margin: var(--spacing-xl) 0; padding: var(--spacing-xl); background: #F8FAFC; border-radius: var(--radius-lg); border: 2px solid #E2E8F0;">
          ...
          <div class="results-score" style="margin: 0;">${i.toFixed(2)} <span style="font-size: 20px; color:#94A3B8;">/ ${n}</span></div>
        </div>
    ...
    `,(a=document.getElementById("review-btn"))==null||a.addEventListener("click",()=>{this.currentState="REVIEW",this.currentQuestionIndex=0,this.render()}),(o=document.getElementById("results-leave-btn"))==null||o.addEventListener("click",()=>{document.fullscreenElement&&document.exitFullscreen().catch(l=>console.warn(l)),window.location.href="mock-tests.html"}),window.lucide.createIcons()}}new f;
