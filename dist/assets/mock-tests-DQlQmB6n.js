import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{b as a,q as r,c as l,a as d,o as c}from"./firebase-config-DyDltD9C.js";import{o as h}from"./auth-service-CzQAHVFu.js";import{g as v,a as p}from"./admin-service-B7fwtq3Q.js";class u{constructor(){this.enrolledGrid=document.getElementById("cd-enrolled-grid"),this.availList=document.getElementById("cd-avail-list"),this.dynamicTests=document.getElementById("cd-dynamic-tests"),this.init()}async init(){h(async e=>{const[i,t]=await Promise.all([e?this.loadEnrolled(e.uid):this.showEnrolledGuest(),this.loadAvailableCourses(),this.loadDynamicTests()])})}async loadEnrolled(e){var s;const i=await v(e),t=i.success&&i.courses?i.courses:[];if(t.length===0){this.enrolledGrid.innerHTML=`
        ...
        </div>`;return}this.enrolledGrid.innerHTML=t.map((o,n)=>this.enrolledCard(o,n)).join(""),(s=window.lucide)==null||s.createIcons()}showEnrolledGuest(){this.enrolledGrid.innerHTML=`
      <div style="grid-column:1/-1;text-align:center;padding:18px 12px;">
      ...
      </div>`}async loadAvailableCourses(){var t;const e=await p(),i=e.success&&e.courses?e.courses:[];if(i.length===0){this.availList.innerHTML='<p style="font-size:13px;color:#9ca3af;text-align:center;padding:12px 0;">No courses available yet.</p>';return}this.availList.innerHTML=i.map((s,o)=>this.availCard(s,o)).join(""),(t=window.lucide)==null||t.createIcons()}availCard(e,i){const t=this.catIcon(e.category);return`
      ...
      </a>`}async loadDynamicTests(){var e;try{const i=await a(r(l(d,"practiceTests"),c("createdAt","desc")));if(i.empty)return;const t=i.docs.map(s=>{var n;const o={id:s.id,...s.data()};return`
